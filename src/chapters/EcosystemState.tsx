  ...
  return (
    <div style={{ padding: '0.5rem', background: 'var(--color-bg-secondary)', borderRadius: 0 }}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.4rem' }}>
        <button disabled style={{ padding: '0.3rem 0.6rem', background: 'var(--color-bg-tertiary)', color: 'var(--color-text-muted)', border: 'none', borderRadius: 0, fontSize: '0.8rem' }}>Add Item</button>
        <button disabled style={{ padding: '0.3rem 0.6rem', background: 'var(--color-bg-tertiary)', color: 'var(--color-text-muted)', border: 'none', borderRadius: 0, fontSize: '0.8rem' }}>Remove</button>
      </div>
      <p style={{ fontSize: '0.75rem', color: '#ef4444', marginTop: '0.4rem' }}>Store does not respond to actions</p>
    </div>
  );
}
...
function FixedCartStore() {
  ...
  return (
    <div style={{ padding: '0.5rem', background: 'var(--color-bg-secondary)', borderRadius: 0 }}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.4rem' }}>
        <button onClick={() => { setItems(p => [...p, `Item ${nextId}`]); }} style={{ padding: '0.3rem 0.6rem', background: 'var(--color-accent)', color: '#ffffff', border: 'none', borderRadius: 0, cursor: 'pointer', fontWeight: 600, fontSize: '0.8rem' }}>Add Item</button>
        <button onClick={() => setItems(p => p.slice(0, -1))} disabled={items.length === 0} style={{ padding: '0.3rem 0.6rem', background: items.length ? 'var(--color-bg-tertiary)' : 'var(--color-bg-tertiary)', color: items.length ? 'var(--color-text)' : 'var(--color-text-muted)', border: 'none', borderRadius: 0, cursor: items.length ? 'pointer' : 'default', fontSize: '0.8rem' }}>Remove</button>
      </div>
      <p style={{ fontSize: '0.75rem', color: 'var(--color-success)', marginTop: '0.4rem' }}>Store responds correctly</p>
    </div>
  );
}
